...
# defaults
TANZU_CORE_REPO_HASH="${TANZU_CORE_REPO_HASH:-""}"
...
TANZU_CORE_REPO_BRANCH=${TANZU_CORE_REPO_BRANCH:-$CORE_BUILD_VERSION}
if [[ "${TANZU_CORE_REPO_BRANCH}" != "${CORE_BUILD_VERSION}" ]]; then
    echo "**************** WARNING - TANZU_CORE_REPO_BRANCH = ${TANZU_CORE_REPO_BRANCH} ****************"
fi

rm -rf "${ROOT_REPO_DIR}/core"
set +x
if [[ -n "${TANZU_CORE_REPO_HASH}" ]]; then
...
sed -i.bak -e "s/\$(shell git describe --tags --abbrev=0 2>\$(NUL))/${CORE_BUILD_VERSION}/g" ./Makefile && rm ./Makefile.bak
