...
// these commands are w.r.t knative-serving addon e2e only
func GetAllUpCmds() map[string][]string {
	return map[string][]string{
		"k8s-apply":                  {"apply", "-f", "$"},
		"k8s-get-crds":               {"get", "crds"},
		"k8s-create-ns":              {"create", "ns", "$"},
		"k8s-port-forward":           {"port-forward", `daemonset/knative-serving`, "-n", "knative-serving", `56017:2020`},
		"k8s-daemonset-state":        {"get", "daemonset.apps/knative-serving", "-n", "$", "-o", "$"},
		"tanzu-package-install":      {"package", "install", "$"},
		"k8s-check-pod-ready-status": {"get", "pods", "-l", "$", "-n", "$"},

		"knative-serving-health-check": {"-I", "http://127.0.0.1:56017/api/v1/health"},
	}
}
