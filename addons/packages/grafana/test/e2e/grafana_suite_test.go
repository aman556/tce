...
import (
	...
	"github.com/vmware-tanzu/tce/addons/packages/grafana/test/e2e"
...
func TestGrafanaE2E(t *testing.T) {
	...
	RunSpecs(t, "grafana addon package e2e test suite")
}
...
var _ = BeforeSuite(func() {
	...
	// delete grafana if at all already installed
	// nothing to worry about the err or result here
	cmdHelperDown.CliRunner("tanzu", nil, cmdHelperDown.GetFormatted("tanzu-package-delete", "$", []string{"grafana.tce.vmware.com"})...)

	result, err := cmdHelperUp.CliRunner("tanzu", nil, cmdHelperUp.GetFormatted("tanzu-package-install", "$", []string{"grafana.tce.vmware.com"})...)
	Expect(err).NotTo(HaveOccurred())
